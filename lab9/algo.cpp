...
void sort(Dictionary *dict, int num_w) {
  for (int i = 0; i < num_w - 1; i++) {
      int minI = i;

    for (int j = i + 1; j < length; j++) {

      if (strcmp(dict[j].eng[0], dict[minI].eng[0])<0) {
        minI = j;
      }
    }
    if (strcmp(dict[i].eng[0], dict[minI].eng[0])!=0) {
      swap(dict[i].eng, dict[minI].eng);
      swap(dict[i].rus, dict[minI].rus);
    }
  }
}